...
def eval(args):
    ...
    for _, (img_path, img, pose) in enumerate(dataloader):
        ...
        if args.model == 'hscnet':
            ...
            """
            # =======================================================
            #lbl_1_max = torch.max(lbl_1, dim=1)[0]
            #lbl_2_max = torch.max(lbl_2, dim=1)[0]
            confidence = torch.max(lbl_1, dim=1)[0].cpu().data.numpy()
        ...
        f.close()
        """
        # 1.use 4 points with max confidence to initialize
        # 2.only take the half with larger confidence
        
        #print(confidence.shape)
        
        top_pts = np.argpartition(confidence, -1 * args.num_top)[-1 * args.num_top:]
        #print(top_pts)
        #print(confidence[top_pts])
        
        # Sort arg
        #top_pts = np.flip(top_pts[np.argsort(confidence[top_pts])])
        #print(top_pts)
        #print(confidence[top_pts])
        
        rot, transl = pose_solver.RANSAC_loop(np.reshape(pcoord,
                (-1,2)).astype(np.float64), np.reshape(coord,
                (-1,3)).astype(np.float64), top_pts.astype(np.int32),
                args.num_hyp) 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_top', nargs='?', type=int, default=480,
                        help='number of top confidence')
    args = parser.parse_args()
    print("Number of hypotheses:", args.num_hyp)
    print("Number of top confidence:", args.num_top)
    eval(args)
