...
cdef class pnpransac:
    ...
    def RANSAC_loop(self,
                ...
                np.ndarray[double, ndim=2, mode="c"] obj_pts,
                np.ndarray[int, ndim=1, mode="c"] top_pts,
                int n_hyp):
        cdef float[:, :] img_pts_ = img_pts.astype(np.float32)
        cdef float[:, :] obj_pts_ = obj_pts.astype(np.float32)
        cdef int[:] top_pts_ = top_pts.astype(np.intc)
        cdef int n_pts
        n_pts = img_pts_.shape[0]
        assert img_pts_.shape[0] == obj_pts_.shape[0]
        cdef int n_top
        n_top = top_pts_.shape[0]
        cdef double* pose
        pose = self.c_pnpransac.RANSACLoop(&img_pts_[0,0],
                                        &obj_pts_[0,0],
                                        &top_pts_[0],
										n_top,
                                        n_pts,
