...
def eval(args):
    ...
    for _, (img, pose) in enumerate(dataloader):
        ...
        ransac_end = time.process_time()
        
        print("total 執行時間：%f 秒" % (ransac_end - start))
        print("model 執行時間：%f 秒" % (model_end - start))
        print("ransac執行時間：%f 秒" % (ransac_end - model_end))
        total_times.append(ransac_end - start)
        model_times.append(model_end - start)
        ransac_times.append(ransac_end - model_end)
    ...
    print(np.average(total_times))
    print(np.average(model_times))
    print(np.average(ransac_times))
